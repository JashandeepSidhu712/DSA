...
class LinkedList //The LinkedList class manages the linked list operations.
{
    ...
    void insertAfterGivenValue(int value, int targetValue)
    {
        ...
        while(ptr!=NULL) //Otherwise, it searches for the node with the targetValue in the linked list and inserts the new node after it.
        {
...
int main()
{
    ...
    int value, target;
    cout<<"Enter value to insert and Enter value after which to insert=";
    cin>>value>>target;
    
    obj.insertAfterGivenValue(value, target);
